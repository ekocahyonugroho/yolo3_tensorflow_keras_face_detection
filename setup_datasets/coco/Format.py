...
class YOLO:
    ...
    def coordinateCvt2PASCAL(self,size, box): # Try to set ground-truth bbox with PASCAL format
        dw = size[0]
        dh = size[1]
        ...
        try:
            ...
            for key in data:
                ...
                for idx in range(0, int(data[key]["objects"]["num_obj"])):
                    ...
                    bb = self.coordinateCvt2PASCAL((img_width, img_height), b)

                    cls_id = self.cls_list.index(data[key]["objects"][str(idx)]["name"]) 
                    cls_name = data[key]["objects"][str(idx)]["name"] #uncomment and replace with above if you prefer using ID Class name

                    bndbox = "".join(["".join([str(e), " "]) for e in bb])
                    contents = "".join([contents, str(cls_name), " ", bndbox[:-1], "\n"])
					
                    bndbox2 = "".join(["".join([str(e), ","]) for e in bb])
                    anno = "".join([anno, bndbox2[:-1], ",", str(cls_id), " "])
    ...
    def save_annotation(self, data, annot, save_path, img_path, img_type, manipast_path):
        ...
        try:
            ...
            with open(os.path.abspath(os.path.join(manipast_path, "manifast.txt")), "w") as manipast_file:
                ...
                for key in data:
                    manipast_file.write(os.path.abspath(os.path.join(img_path, "".join([key, img_type," ", annot[key],"\n"]))))
