...
def _main():
    annotation_path = 'datasets/wider_dataset/WIDER_train.txt'
    val_annotation_path = 'datasets/wider_dataset/WIDER_val.txt'  # data
    log_dir = 'logs/wider/'
    classes_path = 'model_data/classes/wider_classes.txt'
    anchors_path = 'model_data/anchors/yolo_anchors_WIDER_train.txt'
    class_names = get_classes(classes_path)
    num_classes = len(class_names)
    anchors = get_anchors(anchors_path)
    anchors_face = get_anchors(anchors_face_path)
    pretrained_path = "model_data/h5_model/darknet53.h5"
    ...
    if True:
        ...
        if not os.path.isfile("bottlenecks.npz"):
            ...
            bottlenecks=bottleneck_model.predict_generator(data_generator_wrapper(lines, batch_size, input_shape, anchors, num_classes, random=False, verbose=True),
             ...
             #steps=(len(lines)//batch_size), max_queue_size=20, workers=1, use_multiprocessing=False, verbose=2)
        ...
        last_layer_model.fit_generator(bottleneck_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes, bottlenecks_train),
                ...
                validation_data=bottleneck_generator(lines[:num_val], batch_size, input_shape, anchors, num_classes, bottlenecks_val),
        ...
        model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                steps_per_epoch=max(1, num_train//batch_size),
                validation_data=data_generator_wrapper(lines[:num_val], batch_size, input_shape, anchors, num_classes),
                validation_steps=max(1, num_val//batch_size),
                epochs=80,
                initial_epoch=30,
...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, random=True, verbose=False):
    ...
    while True:
        ...
        #image_data = np.memmap(image_data)
        if verbose:
            print("Progress: ",i,"/",n)
        box_data = np.array(box_data)
        #box_data = np.memmap(box_data)
